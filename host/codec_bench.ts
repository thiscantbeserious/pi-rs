...
const payloads: { name: string; value: unknown; encodeBytes: number }[] = [
  { name: "Handshake", value: { type: "Handshake", protocol_version: 1, host_pid: 1234 }, encodeBytes: 0 },
  { name: "Heartbeat", value: { type: "Heartbeat" }, encodeBytes: 0 },
  { name: "Shutdown", value: { type: "Shutdown", drain: true }, encodeBytes: 0 },
  {
    name: "EchoRequest-small",
    value: { type: "EchoRequest", request_id: 42, payload: new Uint8Array([1, 2, 3, 4, 5]) },
    encodeBytes: 0,
  },
  {
    name: "ProtocolError",
    value: { type: "ProtocolError", code: "MalformedFrame", message: "bad frame" },
    encodeBytes: 0,
  },
  {
    name: "EchoRequest-1MiB-binary",
    value: { type: "EchoRequest", request_id: 99, payload: largeBinaryPayload(1024 * 1024) },
    encodeBytes: 0,
  },
];
...
async function benchEncode(codec: Codec, value: unknown, iterations: number): Promise<number> {
  for (let i = 0; i < WARMUP; i++) await codec.encode(value);
  const start = performance.now();
  for (let i = 0; i < iterations; i++) await codec.encode(value);
  return performance.now() - start;
}

async function benchDecode(codec: Codec, bytes: Uint8Array, iterations: number): Promise<number> {
  for (let i = 0; i < WARMUP; i++) await codec.decode(bytes);
  const start = performance.now();
  for (let i = 0; i < iterations; i++) await codec.decode(bytes);
  return performance.now() - start;
}
...
console.log(`codec benchmark: ${WARMUP} warmup, encode/decode iterations vary by payload size`);
...
for (const { name, value } of payloads) {
  ...
  const results: Record<string, { encMs: number; decMs: number; encOps: number; decOps: number; bytes: number }> = {};
  let encodedBytes = 0;

  for (const codec of codecs) {
    const encMs = await benchEncode(codec, value, iters);
    const encoded = await codec.encode(value);
    encodedBytes = encoded.byteLength;
    const decMs = await benchDecode(codec, encoded, iters);
    const encOps = opsPerSecond(encMs, iters);
    const decOps = opsPerSecond(decMs, iters);
    results[codec.name] = { encMs, decMs, encOps, decOps, bytes: encoded.byteLength };
    console.log(
  ...
  const combinedRatio = (msgpack.encMs + msgpack.decMs) / (msgpackr.encMs + msgpackr.decMs);
...
console.log(`=== geomean combined ratio (@msgpack/msgpack / msgpackr): ${geomeanRatio.toFixed(2)}x ===`);
console.log(`trigger threshold: 2.00x (above fires the codec swap)`);
if (geomeanRatio > 2) {
  console.log(`RESULT: @msgpack/msgpack LOSES by more than 2x. Fire the codec-swap trigger.`);
} else {
  console.log(`RESULT: @msgpack/msgpack holds (within 2x of msgpackr). Keep ADR 0006 default.`);
}
