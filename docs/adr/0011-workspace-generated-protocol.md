...
- Within pi-core, the agent loop must not depend on the renderer (now in pi-render, ADR 0026): headless mode (ADR 0018) requires the loop to run with no TUI attached - enforce the boundary from the first commit, split into a separate crate if it blurs
