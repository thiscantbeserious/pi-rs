...
- The binding layer must preserve host-local `pi.*` actions that the loader wires in-process rather than over the Host Protocol, notably `pi.exec(command, args, options?)` (host-owned exec via `execCommand`, not a protocol message). Missing it breaks extensions that shell out through the API. See `docs/extension-api-surface.md` Section B
