...
- **Memory is O(total_rendered_lines) in Phase 2, O(total_messages + visible_rendered_lines) in Phase 3.** The message list is O(total_messages). The line cache holds rendered lines for finalized messages. In Phase 2, all messages are cached (no interactive scroll, no eviction), so memory is O(total_rendered_lines). In Phase 3, rendered lines are evicted when scrolled out of view (the cache holds only the visible window plus a margin), so memory drops to O(total_messages + viewport_rendered_lines). Step 7 (replay) reads entries incrementally. This is a deliberate Phase 2 trade-off: simplicity (no eviction logic) over memory, acceptable because Phase 2 sessions are bounded by the replay harness and compaction is Phase 3.
