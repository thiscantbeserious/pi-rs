...
3. **What does the layer actually do?** For terminal output: strip raw ANSI? Escape it? Render it inert via the cell grid (ADR 0004)? For tool execution: OS-level process sandbox? Filesystem allowlist? Network policy? The layer must handle both content-level trust (ANSI injection) and execution-level trust (arbitrary code), but these are very different problems. Is one layer the right shape, or are there two layers (content trust + execution trust) with a shared transparency surface?
