...
- **Metadata fields deferred to Phase 3.** `MessageRef::Assistant` carries `stop_reason` now (it gates the stream-end display). `usage`, `diagnostics`, and `responseModel` are not on `MessageRef` in Step 2. They are deferred to Phase 3 as additive fields on `MessageRef::Assistant` (or a `UsageRef` type), added when the agent loop (the producer) is built and the footer token-counter display is wired. This is an explicit, documented deferral, not a silent gap: the metadata channel (`message` on `MessageUpdate`) is in place; only the typed fields it carries beyond `stop_reason` are deferred.
