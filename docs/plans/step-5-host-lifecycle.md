...
- **Q6 — Mock host structure**: `[[bin]]` `mock-host` at `crates/pi-core/tests/mock_host.rs` (`harness = false`), `--mode` arg: `normal` (handshake, pong, respond to Shutdown), `exit-immediately` (crash-loop test), `go-silent-after-handshake` (Hung test without kill -9).
