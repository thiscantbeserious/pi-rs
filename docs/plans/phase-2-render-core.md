...
- ratatui `Buffer` + `Buffer::diff` (ADR 0024) [[1]](https://docs.rs/ratatui/latest/ratatui/prelude/buffer/struct.Buffer.html). Mode 2026 capability query at startup (`CSI ? 2026 $ p`), graceful degrade where unsupported (P12). BSU/ESU pairs tight around each flush (P12). Real crossterm `FrameSink` impl lands here (Step 2 uses `CountingSink`). Real crossterm `InputSource` impl for the reader thread (ADR 0030) lands here too (Step 2 uses `NullInput`).
